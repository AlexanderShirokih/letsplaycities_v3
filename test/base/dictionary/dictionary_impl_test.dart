  ...
  group('DictionaryServiceImpl', () {
    ...
    group('.getRandomWord()', () {
      test('returns word starting at specified char', () async {
        expect(await DictionaryServiceImpl(map).getRandomWord('t', 0),
            anyOf(["test", "test2"]));
      });

      test('filters by difficulty', () async {
        expect(await DictionaryServiceImpl(map).getRandomWord('w', 1),
            equals("world2"));
      });

      test('returns empty string if no words starting at char', () async {
        expect(await DictionaryServiceImpl(map).getRandomWord('n', 1),
      ...
      test('not uses words city countryCode==0', () async {
        for (int i = 0; i < 10; i++)
          expect(await DictionaryServiceImpl(map).getRandomWord('h', 0),
              isNot(equals("hello2")));
      });
      ...
      test('not uses already used words', () async {
        ...
        for (int i = 0; i < 10; i++)
          expect(await d.getRandomWord('t', 0), isNot(equals("test")));
      });
    });
